...
class FutureChart extends StatefulWidget {
  ...
  FutureChart({this.name, this.symbol, this.AccessToken, this.list});
...
class _FutureChartState extends State<FutureChart> {
  ...
  loadStocks(){
    ...
    print(widget.symbol);
    String ur = "$url"+'/stocks/GetFutureHistory/'+widget.symbol;
    http.get(ur,)
        .then((response) async{
      ...
      setState(() {
        ...
        for(int i = 0; i< list.length; i++){
          var pr = list[i]['last'];
          List priceSplit = pr.split(',');

          var price = double.parse("${priceSplit[0]}${priceSplit[1]}");
